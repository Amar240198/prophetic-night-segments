...
interface WorkspaceValue {
  ...
  result: NightCalculationResult | null;
...
export function PrayerWorkspace({
  ...
  const initialLoad = useRef(loadLivePrayerTimes);
  useEffect(() => {
    if (autoLoad)
      void initialLoad.current({ preventDefault() {} } as React.FormEvent<HTMLFormElement>);
  }, [autoLoad]);
  ...
  const schedule: DailyPrayerSchedule | null = providerInfo?.dailyPrayerTimes
    ? {
        ...
        asr:
          providerInfo.dailyPrayerTimes.asr ?? providerInfo.dailyPrayerTimes.asrStandard ?? "00:00",
      }
    : null;
  const prayers = schedule ? (
    <AllPrayersCard schedule={schedule} syncContext={syncContext} />
  ) : null;
  const settings = (
      ...
      >
        ...
        <label className="grid min-w-0 gap-2 text-sm text-[#c8d4d0]">
          ...
          <select
            ...
            onChange={(event) =>
              setFirstAdhanMinutes(event.target.value === "" ? null : Number(event.target.value))
            }
            className="w-full min-w-0 border border-white/20 bg-[#06151a] px-3 py-3 text-white outline-none focus:border-[#d0ae67] sm:px-4"
          >
  ...
  return (
    <WorkspaceContext.Provider
      value={{
        ...
        result: engineResult,
        syncContext,
        timeZone: submitted?.timeZone ?? "Europe/London",
      }}
    >
      {children}
    </WorkspaceContext.Provider>
